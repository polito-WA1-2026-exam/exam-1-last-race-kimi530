...
import { getRandomStartEnd, saveGame, saveGameSegment, getRandomEvent, getLeaderboard } from "./dao-game.js";
   ...
   let currentLine = null;
    ...
    for (const seg of segments) {
        ...
        if (currentLine === null) {
            ...
            const isInterchange = allSegments
                .filter(valid =>
                    valid.from_id === seg.fromId ||
                    valid.to_id === seg.fromId
                )
                .map(valid => valid.line_id)
...
app.get('/api/game/start', isLoggedIn, async(req , res) => {
  try {
    ...
    if (!result) 
       return res.status(500).json({ error: 'Could not find valid start/end pair' });
        res.json(result);
...
app.post('/api/game/submit', isLoggedIn, async (req, res) => {
    ...
    if (!startId || !endId || !segments || !Array.isArray(segments) || segments.length === 0)
    ...
    try {
        ...
        for (const seg of segmentData) {
            await saveGameSegment(gameId, seg.stepOrder, seg.fromId, seg.toId, seg.eventId, seg.coinsAfter);
        }
